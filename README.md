...
<br>
<p>A substring correspondente ao grupo é salva em uma "variável" temporária, que pode ser acessada dentro do mesmo regex usando uma barra invertida e o número do grupo de captura (por exemplo, \1). Os grupos de captura são numerados automaticamente pela posição de seus parênteses de abertura (da esquerda para a direita), começando em 1. O exemplo abaixo corresponde a uma palavra que ocorre três vezes separada por espaços:</p>
